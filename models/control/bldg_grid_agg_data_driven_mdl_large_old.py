...
class bldg_grid_agg_data_driven_mdl_large:
    ...
    def reinit(self, inputs, disturbances):
        ...
        # Upstream disturbances do not directly affect temperature dynamics, thus Bv = 0
        ...
        self.Dyu_lin = dot(
            np.array(
                [np.zeros(np.shape(self.Dyu)[1]).tolist(), self.Dyu[1], -1*self.Dyu[1]]
            ), 
    ...
    def process_refs_horiz(self, refs, refs_const):
        refs = (
            np.array(refs_const) - np.array(
                [0, self.truth_model_Pwr, -1*self.truth_model_Pwr]*self.horiz_len
            ).reshape(np.shape(refs_const)[0], 1) + np.array(
                [self.Cy_lin]*self.horiz_len
            ).reshape(np.shape(refs_const)[0], 1) + np.array(
                [self.Dyu_lin]*self.horiz_len
            ).reshape(np.shape(refs_const)[0], 1) + np.array(
                [self.Dyd_lin]*self.horiz_len

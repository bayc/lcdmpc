...
class bldg64_control:
    ...
    def reinit(self, inputs, disturbances):
        ...
        # Model matrices
        # self.A = np.array([[0.9748]]) # 5 minute model
        self.A = np.array([[0.9963]]) # 1 minute model
        # coefficients for Q_hvac (-ve value for cooling) - self.mean_inputs(4)
        # self.Bu = np.array([[0.00751, 0.0, 0.0]])  # 5 minute model
        self.Bu = np.array([[0.00153, 0.0, 0.0]]) # 1 minute model
        self.Bv = np.array([[0.0]])
        # self.Bd = np.array([[0.001, 0.00921, 0.00313]])  # 5 minute data
        self.Bd = np.array([[0.000102, 0.001558, 0.00059]]) # 1 minute model
    ...
    def add_var_group(self, optProb):
        ...
        optProb.addVarGroup('ms_dot', self.horiz_len, type='c', 
                            lower=0.0, upper=2.67, value=0.8)

...
class bldg62_control:
    ...
    def reinit(self, inputs, disturbances):
        ...
        # Model matrices
        # self.A = np.array([[0.9739]]) # 5 minute model
        self.A = np.array([[0.996]]) # 1 minute model
        # coefficients for Q_hvac (-ve value for cooling) - self.mean_inputs(4)
        # self.Bu = np.array([[0.0033, 0.0, 0.0]])  # 5 minute model
        self.Bu = np.array([[0.000738, 0.0, 0.0]]) # 1 minute model
        self.Bv = np.array([[0.0]])
        # self.Bd = np.array([[0.001, 0.0049, 0.0019]])  # 5 minute data
        self.Bd = np.array([[0.0001, 0.000902, 0.000322]]) # 1 minute model
        ...
        self.Bd_mean_inputs = np.array([[21.15], [0.0], [0.0]])
    ...
    def add_var_group(self, optProb):
        ...
        optProb.addVarGroup('ms_dot', self.horiz_len, type='c', 
                            lower=0.0, upper=7.65, value=0.8)
        optProb.addVarGroup('T_sa', self.horiz_len, type='c', 
                            lower=10.0, upper=20.0, value=12.8)

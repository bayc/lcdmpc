...
class bldg63_control:
    ...
    def reinit(self, inputs, disturbances):
        ...
        # Model matrices
        # self.A = np.array([[0.9761]]) # 5 minute model
        self.A = np.array([[0.9967]]) # 1 minute model
        # coefficients for Q_hvac (-ve value for cooling) - self.mean_inputs(4)
        # self.Bu = np.array([[0.00391, 0.0, 0.0]])  # 5 minute model
        self.Bu = np.array([[0.00084, 0.0, 0.0]]) # 1 minute model
        self.Bv = np.array([[0.0]])
        # self.Bd = np.array([[0.001, 0.0067, 0.0026]])  # 5 minute data
        self.Bd = np.array([[0.000, 0.001059, 0.000533]]) # 1 minute model
    ...
    def add_var_group(self, optProb):
        ...
        optProb.addVarGroup('ms_dot', self.horiz_len, type='c', 
                            lower=0.0, upper=5.05, value=0.8)

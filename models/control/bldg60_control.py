...
class bldg60_control:
    ...
    def reinit(self, inputs, disturbances):
        ...
        # self.Bu = np.array([[0.0032, 0.0, 0.0]])  # 5 minute model
        self.Bu = np.array([[0.000732, 0.0, 0.0]]) # 1 minute model
        self.Bv = np.array([[0.0]])
        # self.Bd = np.array([[0.001, 0.0048, 0.0019]])  # 5 minute data
        self.Bd = np.array([[0.0001, 0.000881, 0.00032]]) # 1 minute model

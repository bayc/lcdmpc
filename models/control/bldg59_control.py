...
class bldg59_control:
    ...
    def reinit(self, inputs, disturbances):
        ...
        # Model matrices
        # self.A = np.array([[0.9766]]) # 5 minute model
        self.A = np.array([[0.9967]]) # 1 minute model
        # coefficients for Q_hvac (-ve value for cooling) - self.mean_inputs(4)
        # self.Bu = np.array([[0.0035, 0.0, 0.0]])  # 5 minute model
        self.Bu = np.array([[0.00078, 0.0, 0.0]]) # 1 minute model
        self.Bv = np.array([[0.0]])
        # self.Bd = np.array([[0.001, 0.0057, 0.0029]])  # 5 minute data
        self.Bd = np.array([[0.0001, 0.000948, 0.000539]]) # 1 minute model
